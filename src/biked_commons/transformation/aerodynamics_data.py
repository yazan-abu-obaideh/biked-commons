...
def assemble_aero_data(interface_df, anthropometric_df):
    ...
    combined_df.columns = ['hand_x', 'hand_y', 'hip_x', 'hip_y', 'crank_length', 'upper_leg', 'lower_leg', 'arm_length',
...
def saddle_far_enough_from_bottom_bracket(x):
    return torch.abs(x['lower_leg'] - x['upper_leg']) <= torch.sqrt(x['hip_x'] ** 2 + x['hip_y'] ** 2) - x[
        'crank_length']
...
def hip_angle_over_n_degrees(x, n=10):
    ...
    Tssh = torch.acos(
        (x['torso_length'] ** 2 + Lsh ** 2 - x['arm_length'] ** 2) / (2 * x['torso_length'] * Lsh)) * rad2deg

    # Angle from knee to saddle to closest pedal position
    Tksp = torch.acos((x[:, 1] ** 2 + (torch.sqrt(x['hip_x'] ** 2 + x['hip_y'] ** 2) - x['crank_length']) ** 2 - x[
        'lower_leg'] ** 2) / (2 * x['upper_leg'] * (
                torch.sqrt(x['hip_x'] ** 2 + x['hip_y'] ** 2) - x['crank_length']))) * rad2deg
...
def knee_angle_over_n_degrees(x, n=20):
    ...
    Tskp = torch.acos((ul ** 2 + ll ** 2 - Lsps ** 2) / (2 * ul * ll)) * rad2deg

    return Tskp >= n


def checkconfig(x):
    valid = leg_long_enough(x) & saddle_far_enough_from_handles(x) & saddle_far_enough_from_bottom_bracket(
        x) & hip_angle_over_n_degrees(x) & knee_angle_over_n_degrees(x)
